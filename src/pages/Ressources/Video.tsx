...
export default function Video() {
  ...
  const stats = [
    { value: "+50", label: "Vidéos disponibles", icon: VideoIcon },
  ...
  return (
    ...
    >
      ...
      <div className="grid lg:grid-cols-2 gap-12 items-center mb-16">
        <div>
          <div className="inline-flex items-center gap-2 bg-red-500/10 rounded-full px-3 py-1 mb-4">
            ...
            <span className="text-sm font-medium text-red-500">YouTube & Webinars</span>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            <a
              ...
              className="inline-flex items-center gap-2 bg-red-600 text-white px-6 py-3 rounded-full font-semibold hover:bg-red-700 transition-all"
            >
              <Youtube size={18} />
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-red-500/20 to-brand-500/20 rounded-2xl blur-3xl" />
          <div className="relative bg-gradient-to-br from-red-500/10 to-brand-500/10 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {stats.map((stat, idx) => {
                ...
                return (
                  <div key={idx} className="bg-white dark:bg-dark-800 rounded-xl p-3 text-center shadow-lg">
                    <IconComponent className="w-6 h-6 text-brand-500 mx-auto mb-1" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8" id="videos">
          {filteredVideos.map((video) => (
            <div key={video.id} className="group cursor-pointer" onClick={() => setSelectedVideo(video)}>
              <div className="relative rounded-xl overflow-hidden bg-gradient-to-br from-brand-500/20 to-blue-500/20 transition-transform group-hover:scale-[1.02] duration-300">
                {/* Miniature avec icône */}
                <div className="aspect-video flex flex-col items-center justify-center">
                  <div className="text-6xl mb-2">{video.thumbnail}</div>
                  <div className="text-sm text-gray-500">{video.type}</div>
                </div>
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <div className="w-14 h-14 bg-brand-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:scale-110">
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-gray-400 mt-2">Par {video.author}</p>
                <div className="flex items-center gap-3 mt-3 text-gray-400">
                  <ThumbsUp size={14} className="cursor-pointer hover:text-brand-500 transition-colors" />
                  <Share2 size={14} className="cursor-pointer hover:text-brand-500 transition-colors" />
                  <BookmarkPlus size={14} className="cursor-pointer hover:text-brand-500 transition-colors" />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4" onClick={() => setSelectedVideo(null)}>
          <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            <button 
              ...
              className="absolute -top-12 right-0 text-white hover:text-brand-500 transition-colors"
            >
              ✕ Fermer
            </button>
            ...
            <div className="bg-gray-900 p-4 rounded-b-xl mt-1">
      ...
      <div className="mt-16 bg-gradient-to-r from-brand-500/10 to-blue-500/10 rounded-2xl p-8">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            Prochains webinaires
          </h2>
          <p className="text-gray-600 dark:text-white/60">
            Inscrivez-vous gratuitement à nos prochaines sessions
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-dark-800 rounded-xl p-5 flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              <h3 className="font-bold">SEO 2024 : Les nouvelles tendances</h3>
              <p className="text-sm text-gray-500">25 Avril 2024 - 14h00</p>
            </div>
            <button className="bg-brand-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-brand-600 transition-colors">
              S'inscrire
            </button>
          </div>
          <div className="bg-white dark:bg-dark-800 rounded-xl p-5 flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              <h3 className="font-bold">Social Ads : Maximisez votre ROI</h3>
              <p className="text-sm text-gray-500">2 Mai 2024 - 11h00</p>
            </div>
            <button className="bg-brand-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-brand-600 transition-colors">
              S'inscrire
            </button>
      ...
      <div className="mt-12 bg-gradient-to-r from-red-600 to-red-700 rounded-2xl p-8 text-center text-white">
        ...
        <p className="mb-4 opacity-90">Abonnez-vous à notre chaîne YouTube et activez la cloche pour recevoir toutes nos nouveautés</p>
        ...
        <p className="text-xs opacity-70 mt-4">+1000 abonnés • Nouvelles vidéos chaque semaine</p>
      </div>
    </ResourceLayout>
  )
}